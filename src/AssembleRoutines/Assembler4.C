...
#include "../../include/AssembleRoutines/Assembler4.h"
                      ...
                      std::vector<const TFESpace2D*>& ferhs)
{
    ...
    this->maximum_number_base_function = 0;
    ...
    for(int i=0; i<this->Coll->GetN_Cells(); i++)
    {
        ...
        cell->SetCellIndex(i);
	// find the maximum number of basis function used by the current collection
	for(unsigned int j=0;j<fespaces.size();j++)
	{
	  FE2D CurrentElement = fespaces[j]->GetFE2D(i,this->Coll->GetCell(i));
	  if (N_BaseFunct[CurrentElement]>this->maximum_number_base_function)
	    {
	      this->maximum_number_base_function = N_BaseFunct[CurrentElement];
	    }
	}
    }
    // --------------------------------------------------------------------
    Output::print("Assembler: maximum_number_base_function = ", maximum_number_base_function);
                            ...
                            int AssemblePhaseID)
{
    ...
    {
        aux = new double
        [this->n_all_matrices * maximum_number_base_function * maximum_number_base_function];
        Matrices = new double* [this->n_all_matrices*maximum_number_base_function];
        for(int j=0;j<this->n_all_matrices*maximum_number_base_function;j++)
            Matrices[j] = aux+j*maximum_number_base_function;
					    ...
					    double **Matrices)
{
	...
	FE2D TestElement = ((TFESpace2D *) rectangular_matrices[j]->GetTestSpace())
        ...
        double **Matrix = Matrices+(j+this->n_square_matrices)*maximum_number_base_function;
                                     ...
                                     const Example2D& example)
{
	...
	// local_rhs = LocRhs[j] ?
        //double *local_rhs = righthand+j*maximum_number_base_function; 
