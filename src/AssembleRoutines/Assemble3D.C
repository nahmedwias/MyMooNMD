...
// ########################################################################
  ...
  if(n_sqmatrices)
  {
    ...
    for(i=0;i<n_sqmatrices;i++)
    {
      auto fespace = sqmatrices[i]->GetFESpace3D();
...
// ########################################################################
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    for(j=0;j<n_rhs;j++)
    {
      ...
      N_EdgeDOF = ele->GetFEDesc3D()->GetN_JointDOF();
          ...
          */
          ...
          switch(Cond0)
          {
            ...
            case SLIP_FRICTION_PENETRATION_RESISTANCE:
              ...
              EdgeDOF = ele->GetFEDesc3D()->GetJointDOF(m);
              ...
              for (ii=0;ii<N_;ii++)
              {
                ...
                for (jj=0;jj<N_;jj++)
                {
                  ...
                  if (j==0)
                  {
                    ...
                    if ( TDatabase::ParamDB->INTERNAL_SLIP_WEAK_FORM || //if weak..
                        (!TDatabase::ParamDB->INTERNAL_SLIP_WEAK_FORM && //..or strong with conditions
                            ((fabs(ny)>eps) || (fabs(nz)>eps) || (penetration_penalty<1e3))) )
                    {
                    // update first matrix
                    found = 0;
                  ...
                  if (j==1)
                  {
                    ...
                    if ( TDatabase::ParamDB->INTERNAL_SLIP_WEAK_FORM || //if weak..
                        (!TDatabase::ParamDB->INTERNAL_SLIP_WEAK_FORM && //..or strong with conditions
                         ((fabs(nx)>eps) || (fabs(nz)>eps) || (penetration_penalty<1e3))) )
                    {
                    // update first matrix
                    found = 0;
                    for (ll=RowPtr2[dof_ii];ll < RowPtr2[dof_ii+1]; ll++)
                  ...
                  if (j==2)
                  {
                    ...
                    if ( TDatabase::ParamDB->INTERNAL_SLIP_WEAK_FORM || //if weak..
                       (!TDatabase::ParamDB->INTERNAL_SLIP_WEAK_FORM && //..or strong with conditions
                           ((fabs(nx)>eps) || (fabs(ny)>eps) || (penetration_penalty<1e3))) )
                    {
                    // update first matrix
                    found = 0;
                    for (ll=RowPtr3[dof_ii];ll < RowPtr3[dof_ii+1]; ll++)
...
BoundValueFunct3D **BoundaryValues)
{
  ...
  for(int i = 0; i < N_Cells; ++i)
  {
    ...
    {
      TestElement = matrices[j]->GetTestSpace3D()->GetFE3D(i, cell);
      AnsatzElement = matrices[j]->GetAnsatzSpace3D()->GetFE3D(i, cell);
      ...
      int ActiveBound = matrices[j]->GetTestSpace3D()->GetActiveBound();
