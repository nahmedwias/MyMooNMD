...
//==============================================================================
void LocalAssembling2D::set_parameters_for_tnse_SUPG(LocalAssembling2D_type type)
{
...
//==============================================================================
void LocalAssembling2D::set_parameters_for_tnse_SMAGORINSKY(LocalAssembling2D_type tyep)
{
  int nstype = TDatabase::ParamDB->NSTYPE;
  if(nstype < 4)
...
void LocalAssembling2D::set_parameters_for_tnse_TwoPhase(LocalAssembling2D_type type)
{
  ...
  else if (this->twophase_tnse == 1)
  {
    ...
    this->N_FEValues = 9;
    this->BeginParameter = {0};
    this->FEValue_MultiIndex = { D00, D00, D00, D00, D00, D00, D01, D10, D01};
    this->FEValue_FctIndex = { 0, 1, 2, 3, 5, 6, 5, 5, 6 };
    }
...
//    /* THIS PART USES DIRECTLY PHI FOR COMPUTATION OF CSF */
...
//    // 2 is rho, 4 is phi from which we need derivatives for CSF
    ...
    /* THIS PART USES GRAD PHI FOR COMPUTATION OF CSF */
    this->FEValue_MultiIndex = { D00, D00, D00, D01, D10, D01 };
    this->FEValue_FctIndex = { 2, 5, 6, 5, 5, 6 };
    // 2 is rho, 5 is grad_phix, 6 is grad_phiy, note that
    // crossed derivative has two choices: 5 D01 or 6 D10
    }
  }
  // *********************************************************
  // ************************************************************
