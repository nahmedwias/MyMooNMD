        ...
        for(unsigned int k=0;k<quadPoints.size();k++)
        {
            ...
            if ( (ux_values[0]*nx + uy_values[0]*ny)<0 )
            {
              ...
              /*Output::print(" u(",x_middle,",",y_middle,") = ",
                            ux_values[0],",",uy_values[0], ", normal = ",nx, "," , ny);*/
            }
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
void BoundaryAssembling2D::matrix_dtu_dtv_backflow_stab(BlockFEMatrix &M,
                                                    const TFESpace2D *U_Space,
                                                    std::vector< TFEFunction2D* >& u_conv,
                                                    int boundary_component_id,
                                                    double mult)
{
    ...
    TCollection *coll= U_Space->GetCollection();
    coll->get_edge_list_on_component(boundary_component_id,boundaryEdgeList);
    matrix_dtu_dtv_backflow_stab(M,U_Space,u_conv,boundaryEdgeList,mult);
}
                                                    ...
                                                    double beta)
{
    ...
    for(size_t m=0;m< boundaryEdgeList.size(); m++)
    {
        TBoundEdge *boundedge = boundaryEdgeList[m];
        TBaseCell *cell = boundedge->GetNeighbour(0);
        double h_cell = cell->GetDiameter();
        // get basis dimension and FE space data of cell i
        FE2D FEId = U_Space->GetFE2D(0,cell );
       ...
       get_quadrature_formula_data(quadPoints, quadWeights, LineQuadFormula);
        ...
        for(unsigned int k=0; k<quadPoints.size(); k++)
        {
            ...
            scale_factor = beta/2.0*absneg_u_n*scale_factor*joint_length*joint_length;
            ...
            for(unsigned int l1=0;l1< uorig[k].size();l1++)
              {
                ...
                // compute tangential derivative of test funcion
                double vx_x = uxorig[k][l1];
                double vy_x = vx_x; // x and y component have the same FE space
                double vx_y = uyorig[k][l1];
                double vy_y = vx_y; // x and y component have the same FE space

                double tgt_der_vx = tgx*vx_x + tgy*vx_y;
                double tgt_der_vy = tgx*vy_x + tgy*vy_y;
                
                // loop on ansatz functions
                for(unsigned int l2=0; l2<uorig[k].size(); l2++)
                {
                    int ansatz_DOF = DOF[l2];
                    double ux_x = uxorig[k][l2];
                    double uy_x = ux_x; // x and y component have the same FE space
                    double ux_y = uyorig[k][l2];
                    double uy_y = ux_y; // x and y component have the same FE space

                    double tgt_der_ux = tgx*ux_x + tgy*ux_y;
                    double tgt_der_uy = tgx*uy_x + tgy*uy_y;
                
                    
                    blocks[0]->add(test_DOF, ansatz_DOF, scale_factor*tgt_der_vx*tgt_der_ux ); // A11
                    blocks[4]->add(test_DOF, ansatz_DOF, scale_factor*tgt_der_vy*tgt_der_uy ); // A22
                }
