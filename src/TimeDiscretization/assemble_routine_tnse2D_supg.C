                       ...
                       double ***LocMatrices, double **LocRhs)
{
  ...
  double stab_param[2];
  stab_params(hK,param,c0,coeff[19],stab_param);
  tau_m = stab_param[0];
  tau_c = stab_param[1];
               ...
               double** LocRhs)
{
  ...
  if(TDatabase::ParamDB->P1 == 100)
  {
    ...
    TDatabase::ParamDB->P14 = tau_m;
    TDatabase::ParamDB->P15 = stab_param[1];
  }
  else
  {
    double stab_param[2];  
      ...
      double** OrigValues, int* N_BaseFuncts, double*** LocMatrices, double** LocRhs)
{
  ...
  for(int i=0; i<N_U; ++i)
  {
    ...
    double usigmagradv = tau_m * (u1sigma*test10+u2sigma*test01);
    // right hand side 
    // standard terms 
    Rhs1[i] += Mult*(test00+usigmagradv)*c1;
    Rhs2[i] += Mult*(test00+usigmagradv)*c2;
    // velocity-velocity blocks
    for(int j=0; j<N_U; ++j)
    {
      ansatz10 = Orig0[j];
      ...
      // Galerkin part
      double val  = c0*(test10*ansatz10 + test01*ansatz01) // diffusion term
                     + usigmagradu*test00; // convective term
      // supg contribution
      val +=  (-c0*(ansatz20 + ansatz02) + usigmagradu ) *usigmagradv;
      // grad div contribution
      ...
      // weighted mass matrix
      val = ansatz00 * (test00 + usigmagradv);
      MatrixM[i][j] += Mult * val;      
    }// endfor j<N_U
    ...
    for(int j=0; j<N_P; ++j)
    {
      ...
      // pressure term
      MatrixB1T[i][j] += Mult * (-ansatz00 * test10 + ansatz10 * usigmagradv);
      
      MatrixB2T[i][j] += Mult * (-ansatz00 * test01 + ansatz01 * usigmagradv);
    }
  }
 ...
 double** OrigValues, int* N_BaseFuncts, double*** LocMatrices, double** LocRhs)
{
  ...
  for(int i=0; i<N_U; ++i)
  {
    ...
    for(int j=0; j<N_P; ++j)
    {
      ...
      // pressure term
      double val  = -ansatz00 * test10; // Galerkin term
      val +=  ansatz10 * usigmagradv; // SUPG term (grad p, u grad v)
      MatrixB1T[i][j] += Mult * val;
      
      val  = -ansatz00 * test01; // Galerkin term
      val +=  ansatz01 * usigmagradv; // SUPG term (grad p, u grad v)
      MatrixB2T[i][j] += Mult*val;
    }
  }
}
 ...
 double** OrigValues, int* N_BaseFuncts, double*** LocMatrices, double** LocRhs)
{
  ...
  for(int i=0; i<N_U; ++i)
  {
    ...
    // right hand side 
    // standard terms 
    Rhs1[i] += Mult*c1*(test00 + usigmagradv);
    Rhs2[i] += Mult*c2*(test00 + usigmagradv);    
  }
}
//================================================================================
