...
void matrices_a_tilde2D(std::vector<std::shared_ptr<FEMatrix>> &a,
                        std::shared_ptr<FEMatrix> g, std::shared_ptr<FEMatrix> gt,
                        std::shared_ptr<FEMatrix> m, std::string flag)
{
  int nActive = a[0]->GetAnsatzSpace2D()->GetN_ActiveDegrees();
  int nDof = a[0]->GetAnsatzSpace2D()->GetN_DegreesOfFreedom();
...
{
  std::vector<std::shared_ptr<FEMatrix>> A(2);
  A.at(0) = blocks.at(0); // A11
  A.at(1) = blocks.at(4); // A22
//  A.at(2) = blocks.at(10);// A33

  std::shared_ptr<FEMatrix> M    = matrices_vms.at(4);

  // diagonal A tilde matrices
  // (tilde_A11, tilde_A22)
  std::shared_ptr<FEMatrix> GT   = matrices_vms.at(0);
  std::shared_ptr<FEMatrix> G    = matrices_vms.at(2);
  matrices_a_tilde2D(A, G, GT, M, "a11_tilde");

  GT = matrices_vms.at(1);
  G    = matrices_vms.at(3);
  matrices_a_tilde2D(A, G, GT, M, "a22_tilde");

//  GT = matrices_vms.at(1);
//  G    = matrices_vms.at(4);
//  matrices_a_tilde2D(A, G, GT, M, "a33_tilde");

  // off diagonal A tilde matrices
  // tilde_A12
  A.resize(1); A.at(0) = blocks.at(1); // A12
  GT = matrices_vms.at(1); // GT22
  G = matrices_vms.at(2); // G11
  matrices_a_tilde2D(A, G, GT, M, "a12_tilde");
//  // tilde_A1213
...
//  matrices_a_tilde(A, G, GT, M, "a13_tilde");
  //tilde_A21
  A.resize(1); A.at(0)=blocks.at(3);
  GT=matrices_vms.at(0);// GT11
  G=matrices_vms.at(3); // G22
  matrices_a_tilde2D(A, G, GT, M, "a21_tilde");
//  //tilde_A23
...
void ComputeVMSProjection2D(
  const std::array< std::shared_ptr< FEMatrix >, int(3) > matrixG,
...
//  double* u3=velocity.GetComponent(2)->GetValues();
  ...
  int n=matrixG.at(2)->GetN_Rows();
  std::vector<double> value(n, 0.);
  // G11 * u1
  matrixG.at(0)->multiply(u1, proj,     1.0);
  // G22 * u2
  matrixG.at(1)->multiply(u2, proj+2*n, 1.0);
//  // G33 * u3
...
//  Daxpy(n, 1., &value[0], proj+4*n);
  ...
  if(matrixG.at(2)->GetRowPtr()[n] == n)
  {
    for(int i=0; i<3; ++i)
    {
      for(int j=0; j<n; ++j)
        proj[j+i*n] /= -matrixG.at(2)->GetEntries()[j];
    }
  }
  else
  {
