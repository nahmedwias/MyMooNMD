...
void Mesh::writeToGEO(std::string geoFilename)
{
  ...
  if (triangle.size()) {
    ...
    nVertexPerElem = 3;
    if (quad.size()) {
      Output::print(" Mesh::Write2GEO Error: I cannot write a mesh with ",triangle.size(),
		    " triangles and ",quad.size()," quandrilaterals");
      exit(-1);
    }
  } else  if (quad.size()) {
  ...
  ///@todo check how to write files with elements of different shape
  geofile << "Grid 2D #generated by writeToGEO (A.Caiazzo) " << endl;
  geofile << "Parametrisierung PARXC, PARYC, TMAXC" << endl;
  geofile <<  numberOfElements << " " <<  vertex.size() << " "
	  <<  " 0 " << nVertexPerElem  << " " << nBoundParts;
  geofile <<  "  NEL NVT NMT NVE NBCT" << endl;
  geofile << "DCORVG" << endl;
