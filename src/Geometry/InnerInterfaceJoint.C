...
double TInnerInterfaceJoint::GetArea() const
{
  ...
  if(N_vert == 4){
  //using A=0.5*sqrt(|e|^2*|f|^2-(e,f)^2)
  double norm_e = pow((X[0]-X[2]),2)
                 +pow((Y[0]-Y[2]),2)
                 +pow((Z[0]-Z[2]),2);
  double norm_f = pow((X[1]-X[3]),2)
                 +pow((Y[1]-Y[3]),2)
                 +pow((Z[1]-Z[3]),2);
  double ef = (X[0]-X[2])*(X[1]-X[3])
             +(Y[0]-Y[2])*(Y[1]-Y[3])
             +(Z[0]-Z[2])*(Z[1]-Z[3]);
  return 0.5*sqrt(norm_e*norm_f-ef*ef);
  }
...
{
  // first tangent
  double length = sqrt((X[1]-X[0])*(X[1]-X[0])
                     + (Y[1]-Y[0])*(Y[1]-Y[0])
                     + (Z[1]-Z[0])*(Z[1]-Z[0]));
  tx = (X[1]-X[0])/length;
  ty = (Y[1]-Y[0])/length;
  ...
  length = sqrt((ty*nz - tz*ny)*(ty*nz - tz*ny)
