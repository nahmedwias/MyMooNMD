...
ParameterDatabase TDomain::default_domain_parameters()
{
  ...
  db.add("refinement_final_step_barycentric", false,
   ...
   db.add("manuel_partitioning", false, "this is only true for the "
       "channel with periodic boundaries");
...
#ifdef __3D__
    if(param_db["problem_type"].is(6) && param_db["example"].is(8))
    {
