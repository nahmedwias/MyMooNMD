...
void ChannelFlowRoutines::setZCoordinates(TCollection* Coll, int level)
{
  ...
  for(i=0;i<nCells;i++)
  {
    ...
    for (j=0;j<nVetrices;j++)
    {
      ...
      if (grid_type == 0)
        layer = (atanh(tanh(gamma)*(z-1))/gamma+1)*nLayers/2.0;
      else
        layer = acos(1-z)*nLayers/Pi;
      ...
      if (fabs(layer-layer_int)>1e-5)
      {
        ...
        if (grid_type == 0)
          z = 1 + tanh(gamma*(2.0*k/nLayers -1))/tanh(gamma);
        else
          z = 1-cos(Pi*k/nLayers);
...
void ChannelFlowRoutines::checkZCoordinates(TCollection* Coll, int level)
{
  ...
  {
    if (grid_type==0)
      zcoor[i] = 1 + tanh(gamma*(2.0*i/nLayers -1))/tanh(gamma);
    else
      zcoor[i] = 1-cos(Pi*i/nLayers);
  }
  // first loop over cells
  for(i=0;i<nCells;i++)
  {
