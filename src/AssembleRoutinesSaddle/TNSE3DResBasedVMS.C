...
{  
  ...
  for(int i=0; i<N_U; ++i)
  {
    ...
    for(int j=0; j<N_U; ++j)
    {
      ...
      // second cross term and subgrid term
      val += tau_m * ugradu * (u1+res1) * test010;
      // grad div contribution
      ...
      // second cross term and subgrid term
      val += tau_m * ugradu * (u1+res1) * test001;
      // grad div contribution
      ...
      // second cross term and subgrid term
      val += tau_m * ugradu * (u2+res2)*test100;
      // grad div contribution
      ...
      val += ugradu*ugradv; 
      // second cross term and subgrid term
      val += tau_m * (laplacian + ugradu)*(u2+res2)*test010;
      // grad div contribution
      ...
      // second cross term and subgrid term
      val += tau_m * ugradu * (u2+res2) * test001;
      ...
      // second cross term and subgrid term
      val += tau_m * ugradu * (u3+res3)* test100;
      ...
      // second cross term and subgrid term
      val += tau_m * ugradu * (u3+res3) * test010;
      ...
      // second cross term and subgrid term
      val += tau_m * (laplacian + ugradu) * (u3+res3) * test001;
      Matrix33Row[j] += Mult * val;
      
      // mass matrices
      // standard and supg terms
                       ...
                       double ***LocMatrices, double **LocRhs)
{
  ...
  double **MatrixC   = LocMatrices[18];
  // velocity-pressure blocks
  double **MatrixB1  = LocMatrices[19];
  double **MatrixB2  = LocMatrices[20];
  double **MatrixB3  = LocMatrices[21];
  double **MatrixB1T = LocMatrices[22];
  double **MatrixB2T = LocMatrices[23];
  double **MatrixB3T = LocMatrices[24];
  ...
  for(int i=0; i<N_P; ++i)
  {
    ...
    val =  (u1com/dt + c1)*test100;
    val += (u2com/dt + c2)*test010;
    val += (u3com/dt + c3)*test001;
    ...
    for(int j=0;j<N_U;j++)
    {
      ...
      // supg contribution
      val -= tau_m*(ansatz000 * factor + laplacian + ugradu)*test100;
      MatrixRow1[j] -= Mult*val;
      
      val = -test000*ansatz010;
      // supg contribution
      val -= tau_m*(ansatz000 * factor + laplacian + ugradu)*test010;
      MatrixRow2[j] -= Mult*val;
      
      val = -test000*ansatz001;
      // supg contribution
      val -= tau_m*(ansatz000 * factor + laplacian + ugradu)*test001;
      MatrixRow3[j] -= Mult*val;
    }
