...
void CD2D::solve()
{
  ...
  TSquareMatrix2D *SqMat[1];

  //fill in the matrix block
  //get a non-const pointer to the one block that "matrix" stores
  // TODO must be changed to const as soon as the Solver allows that
  SqMat[0] = s.get_matrix_pointer();

  // TODO change the solver call here as soon as possible
