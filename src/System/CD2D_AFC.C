...
: CD2D (domain, test(param_db), Example_CD2D(param_db), reference_id), alphas_x_i(), old_solution()
{
  ...
  auto& space = *this->CD2D::systems.front().fe_space;
...
/** ************************************************************************ */
void CD2D_AFC::set_AFC_parameters()
{
...
/** ************************************************************************ */
void CD2D_AFC::assemble(const int iteration)
{
  LocalAssembling2D_type t = LocalAssembling2D_type::ConvDiff;
  bool mdml = this->CD2D::solver.is_using_multigrid()
    && this->CD2D::solver.get_multigrid()->is_using_mdml();
  ...
  for(auto & s : this->CD2D::systems)
  {
    ...
    if (db["space_discretization_type"].is("galerkin"))
    {
      ...
      ErrThrow("space_discretization_type only Galerkin because AFC enabled");
  ...
  if (iteration == 0)
  {
    ...
    {
      db["afc_fixed_point_matrix_weight_ori"] = (double)db["afc_fixed_point_matrix_weight"];
      db["afc_fixed_point_derivative_weight_factor"] = 0.0;
    }
...
bool CD2D_AFC::solve(const int iteration)
{
  double t = GetTime();
  CD2D::System_per_grid& s = this->systems.front();
  ...
  if (iteration==0)
  {
    ...
    this->CD2D::solver.solve(s.matrix, s.rhs, s.solution);
  ...
  if(!db["algebraic_flux_correction"].is("none"))
  {
    ...
    if (db["afc_nonlinloop_damping_factor_constant"].is("yes"))
    {
      ...
      if ((db["afc_nonlinloop_anderson_acc"].is("yes"))&&(db["afc_anderson_damping"].is("yes")))
      {
          //double omega = (double) db["afc_nonlinloop_damping_factor"];
          double omega = 0.5;
  ...
  if(is_not_afc_fixed_point_rhs==1)
    this->CD2D::solver.update_matrix(s.matrix);
  this->CD2D::solver.solve(s.rhs, s.solution);
  //Previous Implementation
  ...
  /*if ((db["afc_iteration_scheme"].is("newton"))&&((int)db["afc_nonlinloop_switch_to_newton_scheme"]>=10))
  {
    ...
    if (residual > 100*threshold)
    {
      ...
      {
        db["afc_fixed_point_matrix_weight"] = (double)db["afc_fixed_point_matrix_weight_ori"];
        db["afc_fixed_point_derivative_weight_factor"] = 0.0;
      }
...
/** ************************************************************************ */
void CD2D_AFC::dynamic_damping(const int iteration)
{
  CD2D::System_per_grid& s = this->systems.front();
  double omega = db["afc_nonlinloop_damping_factor"];
  int first_damp=1;
...
/** ************************************************************************ */
void CD2D_AFC::anderson_acceleration_damping(int N_Unknowns, const int iteration, 
...
void CD2D_AFC::output(int i)
{
  ...
  {    
    if(db["algebraic_flux_correction"].is("afc"))
    {
      double epsilon=this->CD2D::example.get_nu();
      Output::print<1>("sigma*L2     : ", setprecision(14), CD2D::errors[0]);
      Output::print<1>("epsilon*H1-semi: ", setprecision(14), std::sqrt(epsilon)*CD2D::errors[1]);     
      Output::print<1>("Energy Norm^2: ", setprecision(14), CD2D::errors[0]*CD2D::errors[0]
                                                  +epsilon*CD2D::errors[1]*CD2D::errors[1]);
    }
  }
}
...
/***************************************************************************/
void CD2D_AFC::do_algebraic_flux_correction(const int iteration, const int is_not_afc_fixed_point_rhs)
{
  Output::print<4>("AFC: enter do_algebraic_flux_correction");
  for(auto & s : this->CD2D::systems)                   // do it on all levels.
  {
