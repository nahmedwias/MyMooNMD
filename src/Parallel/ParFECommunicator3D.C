...
void TParFECommunicator3D::print_info() const
{
  ...
  int root = 0;
  //TODO this method is open for extension adn enhancement in every direction
  ...
  if(my_rank == 0)
  {
    Output::print("STATS (ParFECommunicator3D): Communicator of FESpace ",
                  Mapper->get_fe_space()->GetName());
    int n_m_total = 0;
    for( int i=0 ; i<size ; ++i )
    {
      Output::print(" > Rank ", i, "\t n_masters: ", ns_masters.at(i)); //TODO further fille this output line
      n_m_total += ns_masters.at(i);
    }
    Output::print(" > Total number of dofs in the communicator: ", n_m_total);
  }
}
