      ...
      if(!cell.GetJoint(iJoints)->InnerJoint())
      {
        ...
        if(componentType == DIRICHLET)
        {
          ...
          double horizontalDofsCorrectionValue
              = std::max(fabs(normalX), epsilon);
          double verticalDofsCorrectionValue = std::max(fabs(normalY), epsilon);
              ...
              index < velocity_space->GetBeginIndex()[iCells + 1]; ++index)
          {
            ...
            // OutPut("Horizontal velo dof " << iVeloDof << " in Cell " <<
            // iCells
...
void Saddle_point_preconditioner::fill_augmented_matrix_and_rhs()
{
  ...
  // pressure_mass.get_combined_matrix()->GetStructure().GetN_Entries() << endl;
