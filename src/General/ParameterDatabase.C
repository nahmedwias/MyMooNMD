...
//TODO: there are still some global parameters here
// coming from 'check_parameter_consistency_NSE'
// and 'check_parameter_consistency_CD'
// Once they are removed, remove those 2 includes.
#include <Database.h>
#include <Constants.h>
...
/* ************************************************************************** */
void ParameterDatabase::check_parameters_consistency_NSE()
{
#ifdef _MPI
  int my_rank;
  ...
  if (this->operator[]("space_discretization_type").is("sdfem") && (TDatabase::ParamDB->NSTYPE==1))
  {
      //TDatabase::ParamDB->NSTYPE = 2;
  ...
  if ((this->operator[]("space_discretization_type").is("sdfem")) && (TDatabase::ParamDB->NSTYPE==3))
  {
  ...
  {
    if (!(this->operator[]("space_discretization_type").is("sdfem")))
    {
      this->operator[]("space_discretization_type").set("sdfem");
      if(my_rank==0)
      {
...
*/
  ...
  if (TDatabase::ParamDB->NSE_NONLINEAR_FORM==2||(TDatabase::ParamDB->NSE_NONLINEAR_FORM==4))
  {
    ...
    if ( this->operator[]("space_discretization_type").is("galerkin") )
    {
      this->operator[]("space_discretization_type").set("smagorinsky");
  ...
  {
    if (!(this->operator[]("space_discretization_type").is("vms_projection")||
        this->operator[]("space_discretization_type").is("vms_projection_expl")))
    {
      if(my_rank==0)
      {
  ...
  // LOCAL_PROJECTION
  if (this->operator[]("space_discretization_type").is("local_projection"))
  {
  ...
  if (TDatabase::ParamDB->FLOW_PROBLEM_TYPE == OSEEN)
  {
    ...
    switch (TDatabase::ParamDB->NSTYPE)
    {
      case 1:
        ...
        this->operator[]("space_discretization_type").set("galerkin"); // DISCTYPE = 1
        break;
      case 14:
      if(my_rank==0)
          Output::warn<1>("NSE Parameter Consistency","SUPG/PSPG/grad-div discretization for Oseen because of NSTYPE ",
                      TDatabase::ParamDB->NSTYPE);
        this->operator[]("space_discretization_type").set("supg"); // DISCTYPE = 2
        break;
      default:
      if(my_rank==0)
...
/* ************************************************************************** */
void ParameterDatabase::check_parameters_consistency_CD(int &nonlinear_method)
{
  ...
  if (!this->operator []("space_discretization_type").is("local_projection") &&
      !this->operator []("space_discretization_type").is("local_projection_2_level"))
  {
    // switch off all local projection terms
    TDatabase::ParamDB->LP_FULL_GRADIENT = 0;
    TDatabase::ParamDB->LP_FULL_GRADIENT_COEFF = 0;
  ...
  if ((TDatabase::ParamDB->SDFEM_TYPE != 100)&&(TDatabase::ParamDB->SOLVE_ADJOINT_PROBLEM)&&
      (this->operator []("space_discretization_type").is("sdfem")))
  {
  ...
  if (this->operator []("space_discretization_type").is("gls"))
  {
    this->operator []("space_discretization_type").set("supg");
  ...
  if (this->operator []("space_discretization_type").is("cip"))
  {
    TDatabase::ParamDB->INTERNAL_FACE_INTEGRALS = 1;
    if (TDatabase::ParamDB->CIP_TYPE < 10)
    {
      this->operator []("space_discretization_type").set("galerkin");
      TDatabase::ParamDB->SOLD_PARAMETER_TYPE = 0;
    }
    else
    {
  ...
  if (this->operator []("space_discretization_type").is("dg"))
  {
    this->operator []("space_discretization_type").set("galerkin");
  ...
  if (this->operator []("space_discretization_type").is("local_projection_2_level"))
  {
    nonlinear_method = 1;
    TDatabase::ParamDB->SOLD_PARAMETER_TYPE = 0;
  }
