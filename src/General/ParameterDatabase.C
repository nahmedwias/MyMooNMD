...
ParameterDatabase ParameterDatabase::parmoon_default_database()
{
  ...
  db.add("space_discretization_type", "galerkin",
         "Replaces the global parameter DISCTYPE. Currently supported "
         "are only galerkin and vms_projection. Note that some of these "
         "can work for both NSE and CD. It is important thus to define "
         "this parameter separately in NSE and CD databases.",
