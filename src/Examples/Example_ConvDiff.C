    ...
    case 0: // sine-cosine laplace example
    {
      ...
      if(two_d)
      {
        ...
        f = [diffusion](const Point& point, double, std::vector<double>& coeffs)
        {
          ...
          coeffs[3] = 0;         // reaction
          coeffs[4] = diffusion * 2 * p * p * sin(p * x) * sin(p * y); // rhs
        };
      }
      else // 3D
      {
        ...
        auto e = [](const Point& point, FunctionEvaluation& v)
        {
          ...
          constexpr double p = M_PI; // 2*M_PI;
          v.set(sin(p * x) * sin(p * y) * sin(p * z), 0, MultiIndex::D000);
          v.set(p * cos(p * x) * sin(p * y) * sin(p * z), 0, MultiIndex::D100);
          v.set(p * sin(p * x) * cos(p * y) * sin(p * z), 0, MultiIndex::D010);
          v.set(p * sin(p * x) * sin(p * y) * cos(p * z), 0, MultiIndex::D001);
          v.set(-p * p * sin(p * x) * sin(p * y) * sin(p * z), 0,
                MultiIndex::D200);
          v.set(p * p * cos(p * x) * cos(p * y) * sin(p * z), 0,
                MultiIndex::D110);
          v.set(p * p * cos(p * x) * sin(p * y) * cos(p * z), 0,
                MultiIndex::D101);
          v.set(-p * p * sin(p * x) * sin(p * y) * sin(p * z), 0,
                MultiIndex::D020);
          v.set(p * p * sin(p * x) * cos(p * y) * cos(p * z), 0,
                MultiIndex::D011);
          v.set(-p * p * sin(p * x) * sin(p * y) * sin(p * z), 0,
                MultiIndex::D002);
        };
        ...
        f = [diffusion](const Point& point, double, std::vector<double>& coeffs)
        {
          ...
          coeffs[4] = 0;             // reaction
          coeffs[5] = 3 * p * p * sin(p * x) * sin(p * y) * sin(p * z); // rhs
        };
      }
      break;
    }
