    ...
    case 0: // sine example
    {
      ...
      if(two_d)
      {
        auto u1 = [p](const Point& point, double t, FunctionEvaluation& v)
        {
          ...
          v.set(sin(t) * sin(p * x) * sin(p * y), 0, MultiIndex::D00);
          v.set(p * sin(t) * cos(p * x) * sin(p * y), 0, MultiIndex::D10);
          v.set(p * sin(t) * sin(p * x) * cos(p * y), 0, MultiIndex::D01);
          v.set(-p * p * sin(t) * sin(p * x) * sin(p * y), 0, MultiIndex::D20);
          v.set(p * p * sin(t) * cos(p * x) * cos(p * y), 0, MultiIndex::D11);
          v.set(-p * p * sin(t) * sin(p * x) * sin(p * y), 0, MultiIndex::D02);
        };
        ...
        auto u2 = [p](const Point& point, double t, FunctionEvaluation& v)
        {
          const double x = point.x();
          const double y = point.y();
          v.set(sin(t) * cos(p * x) * cos(p * y), 0, MultiIndex::D00);
          v.set(-p * sin(t) * sin(p * x) * cos(p * y), 0, MultiIndex::D10);
          v.set(-p * sin(t) * cos(p * x) * sin(p * y), 0, MultiIndex::D01);
          v.set(-p * p * sin(t) * cos(p * x) * cos(p * y), 0, MultiIndex::D20);
          v.set(p * p * sin(t) * sin(p * x) * sin(p * y), 0, MultiIndex::D11);
          v.set(-p * p * sin(t) * cos(p * x) * cos(p * y), 0, MultiIndex::D02);
        };
        ...
        auto pr = [p](const Point& point, double t, FunctionEvaluation& v)
        {
          const double x = point.x();
          const double y = point.y();
          v.set(sin(t) * (sin(p * x) + cos(p * y) - 2 / p), 0, MultiIndex::D00);
          v.set(p * sin(t) * cos(p * x), 0, MultiIndex::D10);
          v.set(-p * sin(t) * sin(p * y), 0, MultiIndex::D01);
          v.set(-p * p * sin(t) * sin(p * x), 0, MultiIndex::D20);
          v.set(0., 0, MultiIndex::D11);
          v.set(-p * p * sin(t) * cos(p * y), 0, MultiIndex::D02);
        };
                                                    ...
                                                    double t, double time)
        {
          ...
          {
            case 0:
              return sin(time) * cos(p * t);
              break;
            case 1:
              return -sin(time) * cos(p * t);
              break;
            case 2:
              return -sin(time) * cos(p * (1 - t));
              break;
            case 3:
              return sin(time) * cos(Pi * (1 - t));
              break;
            default:
                                       ...
                                       std::vector<double>& coeffs)
        {
          ...
          coeffs[1] = cos(time) * sin(p * x) * sin(p * y); // time derivative
          // -(1/reynolds)*laplace(u1)
          coeffs[1] += 2 * p * p * sin(time) * sin(p * x) * sin(p * y)
                       / reynolds;
          coeffs[1] += p * sin(time) * cos(p * x); // x-derivative of pressure
          // right hand side, second component
          coeffs[2] = cos(time) * cos(p * x) * cos(p * y); // time derivative
          // -(1/reynolds)*laplace(u2)
          coeffs[2] += 2 * p * p * sin(time) * cos(p * x) * cos(p * y)
                       / reynolds;
          coeffs[2] += -p * sin(time) * sin(p * y); // y-derivative of pressure
          if(!only_stokes)
          {
            // adding nonlinear term
            coeffs[1] += sin(time) * sin(p * x) * sin(p * y) * p * sin(time)
                         * cos(p * x) * sin(p * y);
            coeffs[1] += sin(time) * cos(p * x) * cos(p * y) * p * sin(time)
                         * sin(p * x) * cos(p * y);

            coeffs[2] += sin(time) * sin(p * x) * sin(p * y) * (-p) * sin(time)
                         * sin(p * x) * cos(p * y);
            coeffs[2] += sin(time) * cos(p * x) * cos(p * y) * (-p) * sin(time)
                         * cos(p * x) * sin(p * y);
          }
      ...
      else // 3D
      {
        ...
        auto u1 = [p](const Point& point, FunctionEvaluation& v)
        {
          ...
          v.set(cos(p * x) * sin(p * y) * sin(p * z), 0, MultiIndex::D000);
          v.set(-p * sin(p * x) * sin(p * y) * sin(p * z), 0, MultiIndex::D100);
          v.set(p * cos(p * x) * cos(p * y) * sin(p * z), 0, MultiIndex::D010);
          v.set(p * cos(p * x) * sin(p * y) * cos(p * z), 0, MultiIndex::D001);
          v.set(-p * p * cos(p * x) * sin(p * y) * sin(p * z), 0,
                MultiIndex::D200);
          v.set(-p * p * sin(p * x) * cos(p * y) * sin(p * z), 0,
                MultiIndex::D110);
          v.set(-p * p * sin(p * x) * sin(p * y) * cos(p * z), 0,
                MultiIndex::D101);
          v.set(-p * p * cos(p * x) * sin(p * y) * sin(p * z), 0,
                MultiIndex::D020);
          v.set(p * p * cos(p * x) * cos(p * y) * cos(p * z), 0,
                MultiIndex::D011);
          v.set(-p * p * cos(p * x) * sin(p * y) * sin(p * z), 0,
                MultiIndex::D002);
        };
        ...
        auto u2 = [p](const Point& point, FunctionEvaluation& v)
        {
          ...
          const double y = point.y();
          const double z = point.z();
          v.set(sin(p * x) * cos(p * y) * sin(p * z), 0, MultiIndex::D000);
          v.set(p * cos(p * x) * cos(p * y) * sin(p * z), 0, MultiIndex::D100);
          v.set(-p * sin(p * x) * sin(p * y) * sin(p * z), 0, MultiIndex::D010);
          v.set(p * sin(p * x) * cos(p * y) * cos(p * z), 0, MultiIndex::D001);
          v.set(-p * p * sin(p * x) * cos(p * y) * sin(p * z), 0,
                MultiIndex::D200);
          v.set(-p * p * cos(p * x) * sin(p * y) * sin(p * z), 0,
                MultiIndex::D110);
        ...
        auto u3 = [p](const Point& point, FunctionEvaluation& v)
        {
          ...
          const double y = point.y();
          const double z = point.z();
          v.set(-2 * sin(p * x) * sin(p * y) * cos(p * z), 0, MultiIndex::D000);
          v.set(-2 * p * cos(p * x) * sin(p * y) * cos(p * z), 0,
                MultiIndex::D100);
          v.set(-2 * p * sin(p * x) * cos(p * y) * cos(p * z), 0,
                MultiIndex::D010);
          v.set(2 * p * sin(p * x) * sin(p * y) * sin(p * z), 0,
                MultiIndex::D001);
          v.set(2 * p * p * sin(p * x) * sin(p * y) * cos(p * z), 0,
                MultiIndex::D200);
          v.set(-2 * p * p * cos(p * x) * cos(p * y) * cos(p * z), 0,
                MultiIndex::D110);
          v.set(2 * p * p * cos(p * x) * sin(p * y) * sin(p * z), 0,
                MultiIndex::D101);
          v.set(2 * p * p * sin(p * x) * sin(p * y) * cos(p * z), 0,
                MultiIndex::D020);
          v.set(2 * p * p * sin(p * x) * cos(p * y) * sin(p * z), 0,
                MultiIndex::D011);
          v.set(2 * p * p * sin(p * x) * sin(p * y) * cos(p * z), 0,
                MultiIndex::D002);
        };
