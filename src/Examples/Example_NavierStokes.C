    ...
    case 0: // sine example
    {
      ...
      if(two_d)
      {
        auto u1 = [p](const Point& point, FunctionEvaluation& v)
        {
          ...
          v.set(sin(p * x), 0, MultiIndex::D00);
          v.set(p * cos(p * x), 0, MultiIndex::D10);
          v.set(0., 0, MultiIndex::D01);
          v.set(-p * p * sin(p * x), 0, MultiIndex::D20);
        ...
        auto u2 = [p](const Point& point, FunctionEvaluation& v)
        {
          ...
          v.set(-p * y * cos(p * x), 0, MultiIndex::D00);
          v.set(p * p * y * sin(p * x), 0, MultiIndex::D10);
          v.set(-p * cos(p * x), 0, MultiIndex::D01);
          v.set(p * p * y * cos(p * x), 0, MultiIndex::D20);
          v.set(p * p * sin(p * x), 0, MultiIndex::D11);
          v.set(0., 0, MultiIndex::D02);
        };
        ...
        auto pr = [p](const Point& point, FunctionEvaluation& v)
        {
          ...
          const double y = point.y();
          // isnan(point.z()) should be true, because it is a 2D example
          v.set(sin(p * x) * sin(p * y), 0, MultiIndex::D00);
          v.set(p * cos(p * x) * sin(p * y), 0, MultiIndex::D10);
          v.set(p * sin(p * x) * cos(p * y), 0, MultiIndex::D01);
          v.set(-p * p * sin(p * x) * sin(p * y), 0, MultiIndex::D20);
          v.set(p * p * cos(p * x) * cos(p * y), 0, MultiIndex::D11);
          v.set(-p * p * sin(p * x) * sin(p * y), 0, MultiIndex::D02);
        };
                                                    ...
                                                    double t, double time)
        {
          ...
          {
            case 0:
              return 0.; //-p * 0.y*cos(p*t);
              break;
            case 1:
              return -p * t * cos(p * 1.);
              break;
            case 2:
              return -p * 1. * cos(p * (1 - t));
              break;
            case 3:
              return -p * (1 - t) * cos(p * 0.);
              break;
            default:
              ErrThrow("unknown boundary component");
          }
                                       ...
                                       std::vector<double>& coeffs)
        {
          ...
          coeffs[1] = p * p * sin(p * x)
                      / reynolds;                   // -(1/reynolds)*laplace(u1)
          coeffs[1] += p * cos(p * x) * sin(p * y); // x-derivative of pressure
          // right hand side, second component
          coeffs[2] = -p * p * y * cos(p * x)
                      / reynolds;                   // -(1/reynolds)*laplace(u2)
          coeffs[2] += p * sin(p * x) * cos(p * y); // y-derivative of pressure
          if(!only_stokes)
          {
            // adding nonlinear term
            coeffs[1] += sin(p * x) * p * cos(p * x);
            coeffs[2] += sin(p * x) * p * p * y * sin(p * x)
                         + p * y * cos(p * x) * p * cos(p * x);
          }
