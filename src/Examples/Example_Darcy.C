...
Example Example::Darcy(const ParameterDatabase& param_db)
{
  ...
  double permeability = param_db["permeability_coefficient"];
  ...
  switch(example)
  {
    case 0: // sine-cosine example
    {
      ...
      auto u1 = [](const Point& point, FunctionEvaluation& v)
      {
        ...
        v.set(-p * cos(p * x) * sin(p * y), 0, MultiIndex::D00);
        v.set(p * p * sin(p * x) * sin(p * y), 0, MultiIndex::D10);
        v.set(-p * p * cos(p * x) * cos(p * y), 0, MultiIndex::D01);
        v.set(p * p * p * cos(p * x) * sin(p * y), 0, MultiIndex::D20);
        v.set(p * p * p * sin(p * x) * cos(p * y), 0, MultiIndex::D11);
        v.set(p * p * p * cos(p * x) * sin(p * y), 0, MultiIndex::D02);
      };
      ...
      auto u2 = [](const Point& point, FunctionEvaluation& v)
      {
        ...
        constexpr double p = 2 * M_PI;
        // isnan(point.z()) should be true, because it is a 2D example
        v.set(-p * sin(p * x) * cos(p * y), 0, MultiIndex::D00);
        v.set(-p * p * cos(p * x) * cos(p * y), 0, MultiIndex::D10);
        v.set(p * p * sin(p * x) * sin(p * y), 0, MultiIndex::D01);
        v.set(p * p * p * sin(p * x) * cos(p * y), 0, MultiIndex::D20);
        v.set(p * p * p * cos(p * x) * sin(p * y), 0, MultiIndex::D11);
        v.set(p * p * p * sin(p * x) * cos(p * y), 0, MultiIndex::D02);
      };
    ...
    case 3: // obstacle with lower permeability than surrounding area
    {
      ...
      size_t obstacle = 0; // should be chosen from database
      // inside the obstacle the porosity sigma is multiplied by this factor
      double factor = 10; // should be chosen from database
                                           ...
                                           std::vector<double>& coeffs)
      {
        ...
        switch(obstacle)
        {
          ...
          case 7:
          {
            ...
            if(r < 0.2)
              coeffs[0] *= 1 + 0.5 * (factor - 1) * (cos(5 * M_PI * r) + 1);
            break;
          }
        }
        coeffs[1] = 0.; // right hand side, first component
    ...
    case 4: // 5-spot problem
    {
      ...
      auto p = [r](const Point& point, FunctionEvaluation& v)
      {
        ...
        for(size_t i = 0; i < n_plus; i++)
          value -= log(sqrt(r(x, y, plus[2 * i], plus[2 * i + 1])));
        for(size_t i = 0; i < n_minus; i++)
          value += log(sqrt(r(x, y, minus[2 * i], minus[2 * i + 1])));
        value *= 0.25;
  ...
  Output::dash<2>("The permeability ", permeability);
