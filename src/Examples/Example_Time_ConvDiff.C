  ...
  switch(example)
  {
    case 0: // sine example
    {
      ...
      if(two_d)
      {
        ...
        // exact solution
        auto e = [p](const Point& point, double t, FunctionEvaluation& v)
        {
          const double x = point.x();
          const double y = point.y();
          // isnan(point.z()) should be true, because it is a 2D example
          v.set(sin(t) * (sin(p * x) * sin(p * y) + 1), 0, MultiIndex::D00);
          v.set(sin(t) * p * cos(p * x) * sin(p * y), 0, MultiIndex::D10);
          v.set(sin(t) * p * sin(p * x) * cos(p * y), 0, MultiIndex::D01);
          v.set(-sin(t) * p * p * sin(p * x) * sin(p * y), 0, MultiIndex::D20);
          v.set(sin(t) * p * p * cos(p * x) * cos(p * y), 0, MultiIndex::D11);
          v.set(-sin(t) * p * p * sin(p * x) * sin(p * y), 0, MultiIndex::D02);
        };
        ...
        auto g = [diffusion, p](const Point& point, double time)
        {
          return sin(time) * (sin(p * point.x()) * sin(p * point.y()) + 1);
        };
        bd.push_back(BoundaryData(g));
        // the coefficient function
        lhs_depends_on_time = false;
        rhs_depends_on_time = true;
        f = [diffusion, p](const Point& point, double time,
                           std::vector<double>& coeffs)
        {
          ...
          coeffs[3] = 1.;        // reaction
          coeffs[4] = cos(time)
                      * (sin(p * x) * sin(p * y) + 1); // time derivative
          coeffs[4] += diffusion * 2 * sin(time) * p * p * sin(p * x)
                       * sin(p * y);
          coeffs[4] += coeffs[1] * sin(time) * p * cos(p * x) * sin(p * y);
          coeffs[4] += coeffs[2] * sin(time) * p * sin(p * x) * cos(p * y);
          coeffs[4] += coeffs[3] * sin(time) * (sin(p * x) * sin(p * y) + 1);
        };
      }
      else // 3D
      {
    ...
    case 1: // linear (in space and time) solution
    {
      ...
      if(two_d)
      {
        ...
        // boundary data, automatically computed from known solution
        // bd.push_back(BoundaryData(bc[0], exact[0]));
        auto g = [diffusion, p](unsigned int component, double t, double time)
        {
          switch(component)
          {
    ...
    case 3: // rotating bodies
    {
      ...
      auto e = [](const Point& point, double t, FunctionEvaluation& v)
      {
        ...
        // isnan(point.z()) should be true, because it is a 2D example
        // rotation
        double x0 = cos(t) * x + sin(t) * y - 0.5 * cos(t) - 0.5 * sin(t) + 0.5;
        double y0 = -sin(t) * x + cos(t) * y + 0.5 * sin(t) - 0.5 * cos(t)
                    + 0.5;
        ...
        {
          if(fabs(x0 - 0.5) >= 0.0225 || y0 >= 0.85)
            val = 1.;
        }

        // conical body
        double conical = 1.0 / 0.15 * sqrt((x0 - 0.5) * (x0 - 0.5)
                                           + (y0 - 0.25) * (y0 - 0.25));
        if(conical <= 1.0)
          val = 1. - conical;

        // hump
        double hump = 1.0 / 0.15 * sqrt((x0 - 0.25) * (x0 - 0.25)
                                        + (y0 - 0.5) * (y0 - 0.5));
        if(hump <= 1.0)
          val = 0.25 * (1.0 + (cos(Pi * hump)));
