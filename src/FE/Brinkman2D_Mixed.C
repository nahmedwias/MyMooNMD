...
//WARUM KEINE UNTERSCHEIDUNG IN U1, U2??
// ======================================================================
                               ...
                               double ***LocMatrices, double **LocRhs)
{
    ...
    double K = 1.;
    double alpha = 0.4;//1.;
    double PSPGStab = -alpha*(hK*hK)/(c0*c0+hK*hK); //stabilization = (hK*hK)/(c0*c0+hK*hK) ///warum negativ, wenn positiv geht es schief
    ...
    for(int i=0;i<N_U;i++)
    {
        ...
        for(int j=0;j<N_P;j++)
        {
            ...
            val+= PSPGStab*(ansatz10*test00);                   //stabilization
            
            MatrixB1T[i][j] += Mult * val;
            
            val = -ansatz00*test01;    // -Mult*p*v_y
            val+= PSPGStab*(ansatz01*test00);                   //stabilization
    ...
    for(int i=0;i<N_P;i++)
    {
        ...
        for(int j=0;j<N_U;j++)
        {
            ...
            val = -test00*ansatz10;        // -Mult*q*u_x        ///warum negativ, wenn positiv geht es schief
            val+= PSPGStab * (ansatz00*test10);                 //stabilization
            MatrixB1[i][j] += Mult * val;
            
            val = -test00*ansatz01;        // -Mult*q*u_y        ///warum negativ, wenn positiv geht es schief
                               ...
                               double ***LocMatrices, double **LocRhs)
{
    ...
    double * Rhs_p  = LocRhs[2];            // g_q bzw f_q  (rhs tested with q)
    ...
    double c1 = coeff[1];                 // f1 ; can be set in brinkman2d.dat
    double c2 = coeff[2];                 // f2 ; can be set in brinkman2d.dat
    double c3 = coeff[3];                 // f3 ; can be set in brinkman2d.dat (is the rhs of incompressibility constraint)
    double K = 1.;                        // Permeability
    double alpha = 10;//3;//1;//0;//0.4;//1.;              // PSPG Stabilization Parameter
    double PSPGStab = -alpha*(hK*hK)/(c0*c0+hK*hK); //stabilization = (hK*hK)/(c0*c0+hK*hK)     ///warum negativ, wenn positiv geht es schief
