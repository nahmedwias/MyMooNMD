    ...
    bool nonsymmetric_application)
{
  ...
  if (!sqmatrix.is_square())
  {
    ErrThrow("Matrix must be square!");
  }

  // store the total number of dofs
  int nDofs = sqmatrix.GetN_Rows();

  // check if the size of matrix D fits
  if((int) matrix_D.size() != sqmatrix.GetN_Entries())
  {
    ErrThrow("Incorrect size of matrix_D! ", matrix_D.size(), " != ", sqmatrix.GetN_Entries());
  }
  ...
  // non-const, matrix entries gets modified!
  double* Entries = sqmatrix.GetEntries();
  ...
  // if the artificial diffusion matrix is not yet known, compute it
	if(must_compute_D){
		compute_artificial_diffusion_matrix(sqmatrix, matrix_D);
	}

  // add this matrix to A giving \tilde A (Entries)
  // this is the matrix with the properties of an M matrix
  Daxpy(N_Entries, 1.0, &matrix_D[0], Entries);
  ...
  for(i=0;i<nDofs;i++)
  {
    ...
    for(j=j0;j<j1;j++)
    {
      ...
      index = ColInd[j];
      // d_ij (u_i - u_j)
      F[j] = matrix_D[j] * (sol[index]-sol[i]);
    }
  }
  ...
  for(i=0;i<nDofs;i++)
  {
    ...
    for(j=j0;j<j1;j++)
    {
      ...
      if (Entries[j] > 0)
      {
        ErrThrow("positive entry in FEMTVD ", i, " ", j, " ", Entries[j]);
      }
...
//  if(compute_matrix_D){
//    compute_artificial_diffusion_matrix(*A, matrix_D_Entries);
