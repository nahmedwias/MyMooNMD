...
public:
    ...
    void setPoints_1D(unsigned int points_1D) {
        ...
        xyval_xyderiv_ref_1D_data = std::shared_ptr<double>(new double[3 * 4 * MaxN_QuadPoints_1D * MaxN_BaseFunctions2D], std::default_delete<double[]>());
        {
            double *ptr = xyval_xyderiv_ref_1D_data.get();
            xyval_ref1D[0][0] = &ptr[0];
            xderiv_ref1D[0][0] = &ptr[4 * MaxN_QuadPoints_1D * MaxN_BaseFunctions2D];
            yderiv_ref1D[0][0] = &ptr[2 * 4 * MaxN_QuadPoints_1D * MaxN_BaseFunctions2D];
            for (size_t ii = 0; ii < 4; ii++) {
                for (size_t jj = 0; jj < MaxN_QuadPoints_1D; jj++) {
                    xyval_ref1D[ii][jj] = xyval_ref1D[0][0] + (ii * MaxN_QuadPoints_1D * MaxN_BaseFunctions2D + jj * MaxN_BaseFunctions2D);
                    xderiv_ref1D[ii][jj] = xderiv_ref1D[0][0] + (ii * MaxN_QuadPoints_1D * MaxN_BaseFunctions2D + jj * MaxN_BaseFunctions2D);
                    yderiv_ref1D[ii][jj] = yderiv_ref1D[0][0] + (ii * MaxN_QuadPoints_1D * MaxN_BaseFunctions2D + jj * MaxN_BaseFunctions2D);
                }
            }
        }
        ...
        xieta_ref1D_data = std::shared_ptr<double>(new double[N_BaseFuncts2D * 4 * MaxN_QuadPoints_1D * MaxN_BaseFunctions2D * 3], std::default_delete<double[]>());
        {
...
void NSEErrorEstimator2D::estimate(TFEVectFunct2D &fe_function2D_u, TFEFunction2D &fe_function2D_p, TAuxParam2D &Aux) {
    ...
    TFESpace2D const *fe_spaces[2] = {fe_function2D_u.GetFESpace2D(), fe_function2D_p.GetFESpace2D()};
        ...
        debugimator.GetErrorEstimate((int) derivatives_u.size(), const_cast<MultiIndex2D *>(derivatives_u.data()),
                                     ...
                                     &aux, 2, const_cast<TFESpace2D**>(fe_spaces), eta_K, &maximal_local_error, &estimated_global_error[0]);
# else
        int N_UsedElements, N_LocalUsedElements;
        int N_Points, N_Parameters, N_Edges, N_, N_U, N_P;
        int Used[N_FEs2D];
        int *N_BaseFunct;
        BaseFunct2D *BaseFuncts;
        FE2D LocalUsedElements[N_FEs2D], CurrentElement, CurrentElementP;
...
// ########################################################################
            ...
            FE2D *FEUsedElements;
            for (TFESpace2D *fespace : fe_spaces) {
                FEUsedElements = fespace->GetUsedElements();  /* used finite elements */
                for (size_t j = 0; j < fespace->GetN_UsedElements(); j++) /* for all finite elements */
                {
                    CurrentElement = FEUsedElements[j];
            ...
            std::vector<FE2D> UsedElements((unsigned long) N_UsedElements); /* store used finite elements */
            size_t n_used_elements = 0;                                          /* in array */
            for (size_t i = 0; i < N_FEs2D; i++) {
                if (Used[i]) {
                    UsedElements[n_used_elements] = (FE2D) i;
                    n_used_elements++;
                }
            }// endif
            ...
            if (ee_verbose > 1) {
                ...
                for (size_t i = 0; i < N_UsedElements; i++)
...
// ########################################################################
            ...
            //for (size_t i = 0; i < N_UsedElements; i++)                 // for used finite elements
            {
                CurrentElement = UsedElements[i]; // fe for velocity
                ...
                edgeData.setPoints_1D(N_Points1D);
            ...
            TFESpace2D *fe_space_u = fe_function2D_u->GetFESpace2D();
            int *global_numbers_u = fe_space_u->GetGlobalNumbers();
            int *begin_index_u = fe_space_u->GetBeginIndex();

            TFESpace2D *fe_space_p = fe_function2D_p->GetFESpace2D();
...
// ########################################################################
            ...
            for (size_t i = 0; i < 4; i++)                           // initialize some quantities
...
// ########################################################################
...
#endif
            for (size_t cellIdx = 0; cellIdx < coll->GetN_Cells(); cellIdx++)                      // for all cells on the finest level
            {
                cell = Coll->GetCell((int) cellIdx);                        // next cell
                eta_K[cellIdx] = 0.0;                           // initialize local estimate

                // ####################################################################
                // find local used elements on this cell
                // ####################################################################
                for (size_t j = 0; j < n_fespaces; j++) {
                    CurrentElement = j == 0 ? (fe_space_u->GetFE2D((int) cellIdx, cell)) : (fe_space_p->GetFE2D((int) cellIdx, cell));
                    ...
                    if (j == 0)
                        NeedsSecondDer[j] = TRUE;                      // with 2nd derivative
                    else                                        // for velo
                        NeedsSecondDer[j] = FALSE;
                }
                ...
                CurrentElement = fe_space_u->GetFE2D((int) cellIdx, cell);  // finite element on cell
                BaseFunct = BaseFuncts[CurrentElement];       // basis functions
                N_ = N_BaseFunct[CurrentElement];             // # basis functions
                int *DOF_u = global_numbers_u + begin_index_u[cellIdx];    // dof of current mesh cell
                // cout << "cell no " << i << " begin " << *DOF << endl;
                for (l = 0; l < N_; l++) {
                    FEFunctValues[l] = values_u[DOF_u[l]];    // u fe values of dofs
                    FEFunctValues[l + MaxN_BaseFunctions2D] = values_u[DOF_u[l] + N_U];    // v fe values of dofs
                ...
                // in original mesh cell
                {
                    double **OrigFEValues;
                    double value[2];
                    for (size_t k = 0; k < derivatives_u.size(); k++)            // for all derivatives
                ...
                if (example2D.get_coeffs())                               // get coefficients of pde
                    (example2D.get_coeffs())(N_Points, X, Y, Param, AuxArray); // 0 - eps, 1,2 - rhs
                // prepare 1D quadrature formula
                // pressure first
                CurrentElementP = fe_space_p->GetFE2D((int) cellIdx, cell);  // finite element on cell
                BaseFunctP = BaseFuncts[CurrentElementP];       // basis functions
                N_P = N_BaseFunct[CurrentElementP];             // # basis functions
                int *DOF_p = global_numbers_p + begin_index_p[cellIdx];    // dof of current mesh cell
                //cout << "cell no " << i << " begin " << *DOF_p <<
                // " " << *GlobalNumbersP << " " << BeginIndexP[i] << endl;
                for (l = 0; l < N_P; l++) {
                    FEFunctValues[l + 2 * MaxN_BaseFunctions2D] = values_p[DOF_p[l]];    // p fe values of dofs
...

                {
                    ...
                    {                                       // get values in original cell
                        ...
                        for (size_t j = 0; j < N_Points; j++)               // for all quadrature points
                        {
                            ...
                            {
                                value += FEFunctValues[l + 2 * MaxN_BaseFunctions2D] * Orig[l]; // accumulate p value of derivative in point j
                            }
                ...
                {
                    int n_points1d_val;
                    qf1D->GetFormulaData(n_points1d_val, weights1D, zeta);
                    N_Points1D = (size_t) n_points1d_val;
                }
                ...
                {                                     // get original coordinates of edge quad. points
                    TFEDatabase2D::GetOrigFromRef(RefTrans, (int) N_Points1D, edgeData.xi1D[BaseFunctP][j], edgeData.eta1D[BaseFunctP][j], edgeData.XEdge1D[j].data(), edgeData.YEdge1D[j].data(), edgeData.AbsDetjk1D[j].data());
                    for (size_t k = 0; k < N_Points1D; k++)  // get values and derivatives in original cell
                    {
...

                    {
                        double value;
                        size_t m;
                        for (size_t k = 0; k < N_Points1D; k++)     // for all quadrature points
                        {
                            value = 0;
                            for (l = 0; l < N_P; l++)       // for all basis functions
                            {
                                m = (size_t) (l + 2 * MaxN_BaseFunctions2D);
                ...
                for (size_t j = 0; j < N_Edges; j++)                  // loop over all edges of cell
                {                                     // get original coordinates of edge quad. points
                    TFEDatabase2D::GetOrigFromRef(RefTrans, (int) N_Points1D, edgeData.xi1D[BaseFunct][j],
                ...
                TFESpace2D *fespaces[] = {fe_space_u, fe_space_p};
                // estimate local errors
                EstimateCellError(fespaces, cell, N_Points, X, Y, AbsDetjk, weights, Derivatives,
                                  AuxArray, example2D.get_bc(), example2D.get_bd(),
                                  N_Points1D, zeta, edgeData.XEdge1D, edgeData.YEdge1D, weights1D,
                                  edgeData.xyval_1D, edgeData.xderiv_1D, edgeData.yderiv_1D,
            ...
            for (size_t i = 1; i < 4; i++)  // compute global error estimates
