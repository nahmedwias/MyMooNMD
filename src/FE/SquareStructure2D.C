...
//   FE2D CurrentElement;
//
//   TBaseCell *parent, **children, *child;
//   int N_Children, N_MacroDOF, N_UniqMacroDOF;
//   int MacroDOF[MAXN_CHILDREN*MaxN_BaseFunctions2D];
//   int UniqMacroDOF[MAXN_CHILDREN*MaxN_BaseFunctions2D];
//
//   FESpace = Space;
//
//   ActiveBound=Space->GetN_ActiveDegrees();
...
//   Offset=ActiveBound;
//
//   // loop over all elements: initialise clipboard
//   N_=Space->GetN_Cells();
//   Coll = FESpace->GetCollection();
//   for(i=0;i<N_;i++)
//   {
...
//       if(parent == NULL)
//       {
//         OutPut("no parent cell found!" << endl);
//         exit(-1);
//       }
//
...
//         k = child->GetClipBoard();
//         CurrentElement = Space->GetFE2D(k, child);
//         n1 = TFEDatabase2D::GetFE2D(CurrentElement)->GetN_DOF();
//         N_MacroDOF += n1;
//       }
//
...
//         CurrentElement = Space->GetFE2D(k, child);
//         n1 = TFEDatabase2D::GetFE2D(CurrentElement)->GetN_DOF();
//         Numbers=GlobalNumbers+BeginIndex[k];
//
//         for(l=0;l<n1;l++)
//         {
//           m=Numbers[l];
...
//           else
//           {
//             if(m<HangingBound) HangingAuxPtr[m-Offset]+=N_MacroDOF;
//           } // endif
//         } // endfor l
//
//         // mark cell as already handled;
//         child->SetClipBoard(-1);
//       } // endfor j
//     } // endif ClipBoard >= 0
//   } // endfor i
//
...
//     if(cell->GetClipBoard() >= 0)
//     {
//       // determine all dofs which are connected to a macro cell
//       parent = cell->GetParent();
//       N_Children = parent->GetN_Children();
//
//       N_MacroDOF = 0;
...
//
//       // sort MacroDOF array
//       std::sort(MacroDOF, MacroDOF+N_MacroDOF);
//
//       // condens MacroDOF array into UniqMacorDOF array
//       N_UniqMacroDOF = 0;
//       m = -1;
//       for(l=0;l<N_MacroDOF;l++)
//       {
//         if(m != MacroDOF[l])
//         {
//           UniqMacroDOF[N_UniqMacroDOF] = MacroDOF[l];
//           N_UniqMacroDOF++;
//           m = MacroDOF[l];
//         }
//       }
//
//       for(j=0;j<N_UniqMacroDOF;j++)
//       {
//         for(k=0;k<N_UniqMacroDOF;k++)
//         {
//           m=UniqMacroDOF[k];
//           n=UniqMacroDOF[j];
//           if(n<ActiveBound)
