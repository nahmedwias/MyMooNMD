...
namespace
{
...
#ifdef __3D__
      unsigned int k = 
          (unsigned int) TFEDatabase3D::GetN_BaseFunctFromFE3D(UsedElements[j]);
#else
      unsigned int k 
           = (unsigned int) TFEDatabase2D::GetN_BaseFunctFromFE2D(UsedElements[j]);
#endif
                                     ...
                                     const double *coeff)
  {
    ...
    constant_strong_residual=(6*hK*hK)/epsilon;
    
    if(sigma_0>0)
    {
      inv_sigma=1./sigma_0;
      if(constant_strong_residual>(6*inv_sigma))
        constant_strong_residual=6*inv_sigma;
    }    
 ...
 */
template<int d>
CDErrorEstimator_AFC<d>::CDErrorEstimator_AFC(int type, bool conform_grid,
                                       ...
                                       const std::vector< double >& alphas)
{
  ...
  /****************************************************************************
   * Finding the maximum of hK*hK/|K|. This is used in weights of eta_K
   ****************************************************************************/
  maxK_constant=0;
  double temp;
 ...
 * 
 */
double compute_estimator_weight(double hE, double epsilon, double sigma_0)
{
  double edge_residue_weight;
  edge_residue_weight=(2*hE)/epsilon;
  if(sigma_0>0)
  {
    if(edge_residue_weight>2./(std::sqrt(epsilon*sigma_0)))
    {
      edge_residue_weight=2./(std::sqrt(epsilon*sigma_0));
    }
  }  
 ...
 */
double compute_estimator_weight_d_h(double hE, double epsilon,
                                    double max)
{
  ...
  const std::vector< double >& alphas) const
{
  ...
  const int * ColInd = afc_matrix_D_entries.GetKCol();
  const int * RowPtr = afc_matrix_D_entries.GetRowPtr();
  ...
  result = cell_weight* strong_residual;
  ...
  for(int edgeIdx = 0; edgeIdx < N_Edges_current_cell; edgeIdx++)
  {
    ...
    int *DOF_current=fe_space->GetGlobalDOF(currect_cell_N);
      ...
      || joint->GetType() == IsoBoundEdge)
    {
      ...
      bool shouldNotAbortEstimation = handleJump_BoundaryEdge(
        &boundaryJump, 
        example,  
        N_QuadraturePoints1D, weights1D, 
    ...
    else
    {
      ...
      if (!neigh)
      {
        ...
        for(size_t i = 0; i < N_QuadraturePoints1D; i++)
        {
          ...
          tangent_der_sol+=w*e2*e2*dE*dE*(1-alphaE)*(1-alphaE)*hE;
        }
  ...
  int edgeIdx, const TJoint *joint) const
{
  ...
  switch (Cond0)
  {
    ...
    case NEUMANN:
    {
      ...
      *result += jump * compute_estimator_weight(hE, coeff[0], coeff[3]);
      break;
    }
    case ROBIN:
      // TODO: Robin jump!
