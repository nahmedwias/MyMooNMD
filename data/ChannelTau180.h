...
// with 10% random noise (positive and negative)
  double  noise = 0.1;
  values[0] = 25*z*(2-z)+noise*50*(2*(double)rand()/RAND_MAX-1)/3;
}

void InitialU2(double x, double y, double z, double *values)
{
  double  noise = 0.1;
  values[0] = noise*50*(2*(double)rand()/RAND_MAX-1)/3;
}
			 ...
			 int *N_z_layers, double* &coord_z_layers)
{
  int i,j, k, l, m, n, N_Cells, N_V, *global_numbers, *begin_index, *dof, N_;
    int *N_BaseFunct, max_z_layers = 1000, N_z_layers_help = 0, change;
      ...
      { 		
              switch(j)
	      {
	       ...
	       case  12:
			 ...
			 ly = (((z[3]+3*z[7])/4)+3*((y[2]+3*y[6])/4))/4;
			  ...
			  double *y_dof, double *z_dof)
{
    ...
    double N_U, *u1, *sum_values_per_layer, *average_per_layer;
    ...
    average_per_layer = new double[N_z_layers];
    memset(average_per_layer,0,N_z_layers*SizeOfDouble);
	...
	for (j=0;j<N_z_layers;j++)
	  {
		...
		number_of_sums[j]++;
		if (j==4)
		  OutPut(j << " " << u1[i] << " " << x_dof[i] <<
			  " " << y_dof[i] << " " << z_dof[i] <<endl);
		break;
	      }
	  }
      }
    ...
    for (i=0;i<N_z_layers;i++)
      {
	average_per_layer[i] = sum_values_per_layer[i]/number_of_sums[i]; 
	OutPut("av " << number_of_sums[i] << " " << average_per_layer[i] << endl);
      }


    delete sum_values_per_layer;
    delete number_of_sums;
    delete average_per_layer;
