    ...
    struct System_per_grid
    {
      ...
      */
      ...
      System_per_grid(const Example_TimeNSE2D& example, TCollection& coll,
                      std::pair<int,int> order, Time_NSE2D_Merge::Matrix type);
    };
...
public:
    Time_NSE2D_Merge(const TDomain& domain, const ParameterDatabase& param_db,
...
private:
  ...
  void call_assembling_routine(Time_NSE2D_Merge::System_per_grid& s, 
  ...
  void set_matrices_rhs(Time_NSE2D_Merge::System_per_grid& s, LocalAssembling2D_type type,
        std::vector<TSquareMatrix2D*> &sqMat, std::vector<TMatrix2D*> &reMat,
        std::vector<double*> &rhs);
  /// @brief set the spaces depending on disc types
  void set_arrays(Time_NSE2D_Merge::System_per_grid& s, 
  ...
  void update_matrices_lps(Time_NSE2D_Merge::System_per_grid& s);
