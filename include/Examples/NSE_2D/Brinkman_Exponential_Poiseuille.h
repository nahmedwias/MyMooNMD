...
void ExactU1(double, double y, double *values)
{
  ...
  {
    values[0] = 1/sigma * (1+exp(1/t)-exp((1-y)/t) - exp(y/t)) / (1+exp(1/t));             //u1
    values[1] = 0;                                                                      //u1_x
    values[2] = 1/sigma * (exp((1-y)/t)-exp(y/t))/(t * (1+exp(1/t)));                      //u1_y
    values[3] = 1/sigma * (-exp((1-y)/t)-exp(y/t))/(t * t * (1+exp(1/t)));                 //Delta u1
  }
}
...
void U1BoundValue(int BdComp, double Param, double &value)
{
  ...
  switch(BdComp)
  {
    case 0: value = 0.;
            break;
    case 1: value = 1/sigma * (1+exp(1/t)-exp((1-Param)/t) - exp(Param/t)) / (1+exp(1/t));
            //value = 0; // TEST:sources/sinks,
            break;
    case 2: value = 0.;
            break;
    case 3: value = 1/sigma * (1+exp(1/t)-exp((Param)/t) - exp((1-Param)/t)) / (1+exp(1/t));
            //value = 0; // TEST: sources/sinks,
            break;
