...
void ExactU1(double, double y, double *values)
{
  ...
  {
    values[0] = 0.;//(1./sigma) * (1+exp(1/t)-exp((1-y)/t) - exp(y/t)) / (1+exp(1/t));      
    values[1] = 0.;
    values[2] = 0.;//(1./sigma) * (exp((1-y)/t)-exp(y/t))/(t * (1+exp(1/t)));    
    values[3] = 0.;//(1./sigma) * (-exp((1-y)/t)-exp(y/t))/(t * t * (1+exp(1/t))); 
  }
}
