#ifndef FLOW_AROUND_CYLINDER_H
#define FLOW_AROUND_CYLINDER_H
...
void InitialDU1(double x, double y, double *values)
{
  values[0] = 0;
}

void InitialDU2(double x, double y, double *values)
{
  values[0] = 0;
}

void InitialDP(double x, double y, double *values)
{
  values[0] = 0;
}
// ========================================================================
...
void compute_drag_and_lift(const Time_NSE2D_Merged &tnse2d, double& val)
{
  ...
  const TFEVectFunct2D& u_old(tnse2d.get_velocity_old());
  const TFEFunction2D& p(tnse2d.get_pressure());
  TFEFunction2D *u1 = u.GetComponent(0);
  TFEFunction2D *u2 = u.GetComponent(1);
  TFEFunction2D *u1old = u_old.GetComponent(0);
  TFEFunction2D *u2old = u_old.GetComponent(1);
  
  GetCdCl(u1, u2, &p, u1old, u2old, Cd, Cl);
