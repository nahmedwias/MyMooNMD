...
void ExampleFile()
{
  OutPut("Example: Benchmark3_Neum.h" << endl) ;
}
...
void GetCdCl(const TFEFunction2D *u1fct, const TFEFunction2D *u2fct,
             const TFEFunction2D *pfct, 
             const TFEFunction2D *u1oldfct, const TFEFunction2D *u2oldfct,
             double &cd, double &cl)
{
  int i,j,k,l, N_;
  ...
  const double *u1, *u2, *p, *u1old, *u2old;
...
void compute_drag_and_lift(TFEFunction2D *u1fct, TFEFunction2D *u2fct, TFEFunction2D *pfct,
TFEFunction2D *u1oldfct, TFEFunction2D *u2oldfct)
{
  double Cd, Cl, dP1[4], dP2[4];

  GetCdCl(u1fct, u2fct, pfct, u1oldfct, u2oldfct, Cd, Cl);
