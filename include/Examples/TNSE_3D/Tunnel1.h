...
void BoundCondition(double x, double y, double z, BoundCond &cond)
{
  ...
  {
    // relative error
    if(fabs(((x-px)*nx + (y-py)*ny + (z-pz)*nz)/911.32) < 1e-6)
    {
      return true;
    }
    else 
  ...
  // but we found out that the boundBox is sufficient
  // to choose the boundary conditions. For too fine meshes
  // these might not work and one have to set new bounding boxes.
  ...
  // Output::print("numemann occurance: " , counter_num);
  // Output::print("dirichle occurance: " , counter_dir);
}
