               ...
               double **parameters, double **coeffs)
{
    ...
    for(int i = 0; i < n_points; i++)
    {
        ...
        coeff[i][1] = 0;//-coeff[5] * val_u1[3] - val_p[1] + (coeff[4]/coeff[6]) * val_u1[0];  //(coeff[4]/coeff[6])-1;                       // f1 (rhs of Brinkman problem for u1)
        coeff[i][2] = 0;                                               // f2 (rhs of Brinkman problem for u2)
        coeff[i][3] = 0;                                               // g (divergence term=u1_x+u2_y)
        coeff[i][7] = TDatabase::ParamDB->equal_order_stab_weight_PkPk;
        coeff[i][8] = TDatabase::ParamDB->grad_div_stab_weight;
       }
