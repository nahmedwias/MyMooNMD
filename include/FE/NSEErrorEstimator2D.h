...
protected:
    ...
    void calculateEtaK(const TFEVectFunct2D &fe_function2D_u, const TFEFunction2D &fe_function2D_p, TBaseCell *cell, unsigned int N_Points, unsigned int N_Points1D, double *AbsDetjk, double *weights, double **Derivatives,
                           double **coeffs, const Example2D &example, EdgeData &edgeData, EdgeRefData &edgeRefData, int *global_numbers_u, int *begin_index_u, const double *values_u, int *global_numbers_p, int *begin_index_p,
                           const double *values_p, double *estimated_local_error);
...
public:
    NSEErrorEstimator2D(const Example2D &ex);
    NSEErrorEstimator2D(const Example2D &ex, int type, bool is_nse);

    virtual void estimate(const TFEVectFunct2D &fe_function2D_u, const TFEFunction2D &fe_function2D_p, TAuxParam2D &Aux);
