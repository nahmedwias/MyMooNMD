...
// ======================================================================
  ...
  int N_Active, N_MovVert;
  int **IsoCellEdgeNos;
  
  double *sol, *rhs, *oldrhs, *MeshSol, t1, t2, errors[5], *Iso_refX;
  double tau, end_time, *defect, olderror, olderror1, hmin, hmax, CurrTime, tau1, oldt;
  
  bool UpdateStiffnessMat, UpdateRhs,  ConvectionFirstTime, ReAssembleM=TRUE;
...
//======================================================================    
   ...
   while(TDatabase::TimeDB->CURRENTTIME< end_time)
    {                                               // time cycle
     ...
     SystemMatrix_ALE->GetMeshVelo(N_MovVert, MovBoundVert, Free_Joint, Iso_refX, TDatabase::TimeDB->CURRENTTIME, tau);
#else       
     SystemMatrix_ALE->GetMeshVelo(TDatabase::TimeDB->CURRENTTIME, tau);
#endif      
     ...
     SystemMatrix_ALE->AssembleMARhs(sol, rhs);   
     ...
     if(ReAssembleM)
      { 
       ...
       SystemMatrix_ALE->MoveMesh(CurrTime);
#endif   
       // assemble M Mat in "CurrTime" mesh, only M mat so mesh velo is not needed
       SystemMatrix_ALE->AssembleMRhs(sol, rhs);
      }
