...
#include "../Examples/TNSE_2D/HangingDroplet.h"
...
int main(int argc, char* argv[])
{
  ...
  TBdLine *UpdateAxialBoundTop, *UpdateAxialBound, *UpdateHoriWallBound;
...
//    UpdateHoriWallBound = (TBdLine*)BoundPart->GetBdComp(1);      
   ...
   UpdateAxialBound = (TBdLine*)BoundPart->GetBdComp(5);
...
// ============================================================================================     
     ...
     MapSurfToDomain(IFaceFeFunct[0], FEFunctions_All[6], N_List[0], N_List[1]);      
#endif     
//      PrintSurfSurfactant(N_MovVert[2], MovBoundVert[2], FEFunctions_All[6], N_BData); 
...
//        DecrTimeStepCount++;
  ...
  MovBoundVert[1][0]->GetCoords(SLPX, SLPY);  
  UpdateAxialBoundTop->SetParams(x, y, SLPX-x, SLPY-y);
  for(k=0;k<N_MovVert[0];k++)
   if(k==N_MovVert[0]-1)
    { Bound_Joint[0][k]->UpdateParameters(MovBoundVert[0][k], MovBoundVert[1][0]); }
   else
    { Bound_Joint[0][k]->UpdateParameters(MovBoundVert[0][k], MovBoundVert[0][k+1]); }
         
   // Updating axis boundary (bottom)
   ...
   for(k=0;k<N_MovVert[1];k++)
    if(k==N_MovVert[1]-1)
     { Bound_Joint[1][k]->UpdateParameters(MovBoundVert[1][k], MovBoundVert[2][0]); }
    else
...
//       reparam = TRUE;
//       cout<< "Old N_E " << N_MovVert[2] <<"New N_E " << N_E <<endl;     
//       N_MovVert[2] = N_E;
//       
//       
//       exit(0);
...
//    }
    ...
    MovBoundVert[2][0]->GetCoords(x1, y1);
    ...
    if( ((m % 200)== 0) || (m==1) )
     {
      ...
      PrintSurfSurfactant(N_MovVert[2], MovBoundVert[2], FEFunctions_All[6], N_BData); 
#else
      PrintSurfSurfactant(N_MovVert[2], MovBoundVert[2], NULL, N_BData); 
#endif 
     } //if( ((m % 20)== 0) || (m==1) )
...
//======================================================================    
     if(ReduceTiemStep)
     ...
     else if(IncrTimeStepCount>200)
      {
       ...
       if(tau*1.25<TDatabase::TimeDB->MAX_TIMESTEPLENGTH)
        {
         IncrTimeStepCount=0;  
