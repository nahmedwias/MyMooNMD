...
//  size_t ansatz_order = 2;
  ...
  TFESpace3D feSpace_(grid_collections.back(), (char*) "feSpace_",(char*) "feSpace_", BoundConditionNoBoundCondition, 2);

  //  feSpace_.info();
  //Die Info is not defined in 3D.

  // create some vector which could be seen as a finite element function
  // representation
  BlockVector v1(feSpace_.GetN_DegreesOfFreedom());
  {
    // fill the vector with some values, according to some given function
    TFEFunction3D f(&feSpace_, (char*)"dummy", (char*) "dummy",
  ...
  BlockVector v2(feSpace_.GetN_DegreesOfFreedom());
